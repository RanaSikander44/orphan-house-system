...
auxlib.error = function (message) {
    if (auxlib.DEBUG) console.log ('Error: ' + message);
};
...
auxlib.currencyToNumber = function (currencyStr, currency) {
    ...
    var number = $(tmp).val (currencyStr).maskMoney (currencyInfo).maskMoney ('unmasked')[0];
    $(tmp).remove ();
    return number;
};
...
auxlib.numberToCurrency = function (number, currency) {
    ...
    var str =
        $(tmp).val (number).maskMoney (currencyInfo).maskMoney ('mask').val ()
    $(tmp).remove ();
